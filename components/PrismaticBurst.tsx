...
export type PrismaticBurstProps = {
  ...
  dpr?: number; // Added prop to manually control resolution quality
};
...
// Optimized layered noise (fewer layers if possible, here keeping 5 for visual fidelity)
...
void main(){
    ...
    // OPTIMIZATION: Reduced loop count from 44 to 24.
    // Increased step size slightly to compensate for visual density.
    for (int i = 0; i < 24; ++i) {
        vec3 P = marchT * dir;
        P.z -= 2.0;
        float rad = length(P);
...
const PrismaticBurst = React.memo(({
                                     intensity = 2,
                                     speed = 0.5,
                                     ...
                                     dpr = 1 // Default set to 1 for performance; use 1.5 only for high-end machines
                                   }: PrismaticBurstProps) => {
  ...
  useEffect(() => {
    ...
    // OPTIMIZATION: Default to 1, max at 2. High DPI kills shader performance.
    const finalDpr = Math.min(window.devicePixelRatio || 1, dpr);

    const renderer = new Renderer({
      dpr: finalDpr,
      alpha: false, // Assuming we want a black background for blend modes
      antialias: false, // FXAA or native AA is expensive and often unneeded for noise
      depth: false, // Disable depth buffer (saves memory)
    ...
    const update = (now: number) => {
      ...
      // Skip rendering if not visible or if paused
    ...
    return () => {
      ...
      if (rendererRef.current) {
        ...
        // Try/catch for safety during cleanup
        try {
          if (programRef.current) {
            // OGL handles shader deletion mostly, but good to be explicit if needed
          }
          container.removeChild(gl.canvas);
          // Force lose context if necessary, though rarely needed with OGL's lightness
          const ext = gl.getExtension('WEBGL_lose_context');
          if (ext) ext.loseContext();
        } catch(e) { console.warn("Context cleanup warning", e) }
      }
  ...
  useEffect(() => {
    ...
    if (Array.isArray(colors) && colors.length > 0) {
      const gl = renderer.gl;
      const capped = colors.slice(0, 64); // Limit colors to prevent heavy texture ops
      count = capped.length;
      ...
      gradTex.width = count;
      gradTex.height = 1;
      // Re-upload texture only when needed
